...
# -------------------------------
# （4） 定义函数提取每个 session 中的 tag 得票率
# -------------------------------
def extract_vote_rate_per_session(rank_dict, sessions, tag_name):
    rates = []
    for s in sessions:
        found = False
        for (t, rate) in rank_dict[s]:
            if t == tag_name:
...
cn_values = extract_vote_rate_per_session(tag_rank_sessions_china, cn_sessions, TAG)
jp_values = extract_vote_rate_per_session(tag_rank_sessions_japan, jp_sessions, TAG)
...
ax1.set_ylabel("得票率 (Vote Rate)")
ax1.set_title(f"Tag “{TAG}” 中国区 (第1–11届)")
...
ax2.set_title(f"Tag “{TAG}” 日本区 (第3–20届)")
