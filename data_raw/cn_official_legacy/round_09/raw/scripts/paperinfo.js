...
$(document).ready(function(){
	...
	$('.container').on('click', '.type-change-row', function(){
		...
		if (thischart == "progress" || thischart == "progress-sum") {
			var word_prefix = "新增"
			if (thischart == "progress-sum") {
				word_prefix = "总"
			}
			if (now == "total") {
				chart[thischart].dispatchAction({
					type: 'legendUnSelect',
					name: word_prefix + '角色单项票数'
				});
				chart[thischart].dispatchAction({
					type: 'legendUnSelect',
					name: word_prefix + '音乐单项票数'
				});
				chart[thischart].dispatchAction({
					type: 'legendUnSelect',
					name: word_prefix + '作品单项票数'
				});
				chart[thischart].dispatchAction({
					type: 'legendUnSelect',
					name: word_prefix + '组合单项票数'
				});
				chart[thischart].dispatchAction({
					type: 'legendSelect',
					name: word_prefix + '角色投票人数'
				});
				chart[thischart].dispatchAction({
					type: 'legendSelect',
					name: word_prefix + '音乐投票人数'
				});
				chart[thischart].dispatchAction({
					type: 'legendSelect',
					name: word_prefix + '作品投票人数'
				});
				chart[thischart].dispatchAction({
					type: 'legendSelect',
					name: word_prefix + '组合投票人数'
				});
			} else {
				chart[thischart].dispatchAction({
					type: 'legendSelect',
	...
	$.get('api.php?action=make&object=votedate&text=paper').done(function (data) {
		var data1 = data.sum, data2 = data.single;
		option = {
				title: {
					text: '投票随时间跨度图表（累计）',
					show: false
		...
		$.each(data1.item.list, function(i, item) {
			option.series[i] = {
							name: data1.item.item[i],
							type: 'line',
							data: data1.data[item]
						};
		});
		chart['progress-sum'] = echarts.init($('#progress-sum-chart')[0]);
		chart['progress-sum'].setOption(option);
		$('#progress-sum-chart').data({'other':'total','now':'vote'});
		chart['progress-sum'].dispatchAction({
			type: 'legendUnSelect',
			name: '总角色单项票数'
		});
		chart['progress-sum'].dispatchAction({
		...
		$.each(data2.item.list, function(i, item) {
			option.series[i] = {
							name: data2.item.item[i],
							type: 'line',
							data: data2.data[item]
						};
		});
	...
	chart['geo2'] = echarts.init($('#geo2-chart')[0]);
	$.get('api.php?action=make&object=votegeo&text=all').done(function (data) {
		...
		$.each(data.data.all, function(i, v) {
			temp.all[v.name] = v.value;
		});
		$.each(data.data.male, function(i, v) {
			temp.male[v.name] = v.value;
		});
		$.each(data.data.female, function(i, v) {
			temp.female[v.name] = v.value;
		});
		for (var i = 0; i <= 33; i++) {
			geolist.all[i] = {
				value: (typeof(temp.all[data.item[i]]) == "undefined"? 0 : temp.all[data.item[i]]),
				name: data.item[i]
			};
			geolist.sort[i] = {
				value: (typeof(temp.all[data.item[i]]) == "undefined"? 0 : temp.all[data.item[i]]),
			...
			geolist.male[i] = {
				value: (typeof(temp.male[data.item[i]]) == "undefined"? 0 : temp.male[data.item[i]]),
				name: data.item[i]
			};
			geolist.female[i] = {
				value: (typeof(temp.female[data.item[i]]) == "undefined"? 0 : temp.female[data.item[i]]),
				name: data.item[i]
			};
			geolist.item[i] = data.item[i];
		}
		function sortValue(a, b) {  
			return b.value - a.value;
		}
		geolist.sort.sort(sortValue);
		...
		paperoption['geo2'] = option;
		chart['geo2'].setOption(option['pie']);
		$('#geo2-chart').data({'other':'bar','now':'pie'});
		chart['geo2'].resize();
	...
	crossDiv.on("click", "#button", function() {
		...
		$.post('api.php?action=make&object=votepaper&text=paper', { quest1: quest1, quest2: quest2 }).done(function (data) {
			...
			$("#cross-chart").show("slow",function(){
				var v = data.data;
