...
超过 GitHub 单文件限制的两个问卷 CSV 已按完整记录切为约 80 MB 一卷：
`cn_advanced_questionnaire.csv.part-001` 至 `.part-008`，以及
`cn_legacy_advanced_questionnaire.csv.part-001` 至 `.part-005`。每卷都包含表头，
旁边的 `.parts.json` 记录原文件大小、SHA-256 和分卷校验值。完整文件名仍作为
逻辑名称使用，查询器和构建脚本会自动按编号合并读取。
...
共 33,533 行排行、94 行票数汇总。详细分组行数见 `manifest.json` 的 `row_counts_by_region_round_category`。CN10 与 CN11 的普通榜完整性已单独在 manifest 中核验；高级问卷查询仍按官方可用接口范围记录缺口，不将缺失格伪造为 0。
...
同人曲投票窗口、问卷关联、CP/组合和角色×音乐交叉分析不是本目录的输入排行字段，而是在 `vote_explorer/data/` 中由 `scripts_pipeline/build_vote_explorer_analysis_data.py` 离线生成。旧的 CN1–9/JP3–21 快照已移入 `archive_legacy/vote_datasets/`，不要与当前数据混用。
